...
Шесть пресетов явно заполняют все поля, сбрасывают отсутствующие optional-параметры и отправляют реальный запрос. Активный пример выделен, его параметры доступны в подсказке кнопки. После ответа на пресет страница прокручивается к результату; при reduced motion прокрутка без анимации. При ручном редактировании выделение примера снимается.

Четыре баннера различают полную тройку, shortfall, `no_eligible` и `no_category`; счётчики и причины берутся из ответа API. Сразу ниже видны влияние даты, правила ранжирования и переносимая цепочка pipeline. После карточек сохранена строка остальных eligible. Все отклонённые профили и их причины доступны в раскрываемом списке.

Карточка содержит позицию, стартовую цену, полное объяснение, заметные synthetic/imputed-флаги, языки, часы и форматы. Форматы передаются серверной страницей из того же CSV по ID, без нового API-запроса и без повторной фильтрации на клиенте. Выделенный формат и контекст баннера относятся к отправленному запросу, а не к последующим правкам формы. Для AI показана уже проверенная цитата; подпись подтверждает источник текста, не качество подрядчика. Для fallback показано «Объяснение по данным профиля».

Загрузка блокирует повторную отправку и показывает три скелетона. Ошибки выделены отдельно; «Повторить» отправляет текущие поля формы. Начальное состояние сообщает, что подбор ещё не выполнен. В CSS форма становится одноколоночной ниже 640 px, карточки и чипы переносятся. Браузерный визуальный просмотр в этой среде недоступен: отсутствие обрезания и горизонтального скролла требует ручной проверки, visual PASS не заявлен.
