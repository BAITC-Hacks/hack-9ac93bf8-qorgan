...
Первые шесть строк доступны кнопками над формой: кнопка заполняет поля и отправляет настоящий `POST /api/recommend`. A/A2/B вводятся в обычной форме или через API. Пустые длительность и язык не передаются. Результаты таблицы проверены на реальном CSV и локальном HTTP API.
...
Публичные GET страницы и POST API работают без Vercel login. CSV доступен в serverless runtime без дополнительного file tracing. Проверенный production Demo A: **4353 мс**, повторный AI smoke **3973 мс**, 2 принятых AI-факта и одна fallback-карточка. Это одиночные измерения до добавления кеша, не гарантия latency.
