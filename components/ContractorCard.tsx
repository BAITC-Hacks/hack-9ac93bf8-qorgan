  ...
  return (
    <article className="contractor">
      <div className="card-heading">
        <h3><span className="position" aria-label={`Позиция ${position + 1}`}>{["①", "②", "③"][position]}</span> {card.anon_name}</h3>
      ...
      {card.explanationSource === "ai" && card.explanationEvidence?.trim() ? (
        <p className="evidence">✓ Проверено по описанию: «{card.explanationEvidence}»</p>
      ) : (
        <p className="explanation-source">Объяснение по данным профиля</p>
      )}
